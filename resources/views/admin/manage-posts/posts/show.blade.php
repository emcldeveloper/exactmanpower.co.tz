...
<div class="main-container-middle">
    <div class="container-sidebar bg-white border-left border-right">
        <div class="main-container-middle">
            <div class="container-summary-sidebar border-bottom d-flex p-3">
                <div class="d-block">
                    <div class="h5 mb-3">
                    <!-- <a href="{{ url($route . '/list') }}" class="btn btn-outline-primary mr-3" title="Back to posts list"><i class="fas fa-arrow-left"></i></a> -->
                    <span>{{ $title }}</span>
                    </div>
                    ...
                    <div class="container-detail" scroll-container>
                        <div class="clearfix">
                            ...
                            <div class="border-bottom p-2">
                                <a href="{{ url($route . '/show/'.$row->id.( request('sub_page')? '/'.request('sub_page'): '' )) }}">{{ $row->post_title }}</a>
    ...
    <div class="container-header" style="overflow:visible;">
        <div class="d-flex align-items-center justify-content-between p-3">
            <div class="h5 m-0">
                <span>{{ $title }} info ( {{ $model_info->post_title }} )</span>
            </div>
            
            <div>
                <!-- <a href="{{ url($route . '/edit/'.$model_info->id) }}" class="btn btn-primary"><i class="fas fa-pencil-alt mr-1"></i> Edit</a> -->
                <a href="{{ url($route . '/create') }}" class="btn btn-dark"><i class="fas fa-plus-circle mr-1"></i> Add Post</a>
                <!-- <a href="{{ url($route . '/delete/'. $model_info->id) }}?redirect={{ url($route . '/list') }}" class="btn btn-danger" data-confirmation='I you sure, you want to delete "{{ $model_info->post_title }}"?'><i class="fas fa-trash mr-1"></i> Delete</a> -->
                <div class="d-inline-block dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" id="dropdown-add-another">
                        ...
                        <div class="dropdown-item" data-toggle="modal" data-target="#model_new_reported_posts" >Add reported post</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="container-navbar bg-white border-bottom" style="overflow:visible;">
        <nav class="nav">
            ...
            <a class="nav-link {{ Request::is($route . '/show/*/account-orders')? 'active': null }}" href="{{ url($route . '/show/' . $model_info->id . '/account-orders') }}">Account Orders</a>
            <a class="nav-link {{ Request::is($route . '/show/*/notifications')? 'active': null }}" href="{{ url($route . '/show/' . $model_info->id . '/notifications') }}">Notifications</a>
            <a class="nav-link {{ Request::is($route . '/show/*/post-extras')? 'active': null }}" href="{{ url($route . '/show/' . $model_info->id . '/post-extras') }}">Post Extras</a>
            <a class="nav-link {{ Request::is($route . '/show/*/post-medias')? 'active': null }}" href="{{ url($route . '/show/' . $model_info->id . '/post-medias') }}">Post Medias</a>
            <a class="nav-link {{ Request::is($route . '/show/*/posts')? 'active': null }}" href="{{ url($route . '/show/' . $model_info->id . '/posts') }}">Posts</a>
            <div class="nav-link dropdown">
                <a class="dropdown-toggle" data-toggle="dropdown" href="javascript:;">More</a>
                <div class="dropdown-menu">
                    
            <a class="dropdown-item {{ Request::is($route . '/show/*/reported-posts')? 'active': null }}" href="{{ url($route . '/show/' . $model_info->id . '/reported-posts') }}">Reported Posts</a>
                </div>
            </div>
        </nav>
    </div>
    <div class="container-detail bg-white">
        <div class="clearfix">
            {!! \App\Handlers\Admin\ManagePost\Post\ShowFormHandler::handler(request(), new \App\Models\Post(), $model_info->id) !!}
        </div>
    </div>
</div>
<!-- Modal -->
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content" style="border:4px solid #1193d7;">
            ...
            <div class="modal-body">
                <div class="clearfix">
                {!! \App\Handlers\Admin\AccountOrder\CreateFormHandler::handler(request(), url()->full(), ['post_id'=>$model_info->id]) !!}
                </div>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="model_new_notifications" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true" style="background:rgba(0,0,0,.2);">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content" style="border:4px solid #1193d7;">
            ...
            <div class="modal-body">
                <div class="clearfix">
                {!! \App\Handlers\Admin\Notification\CreateFormHandler::handler(request(), url()->full(), ['post_id'=>$model_info->id]) !!}
                </div>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="model_new_posts" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true" style="background:rgba(0,0,0,.2);">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content" style="border:4px solid #1193d7;">
            ...
            <div class="modal-body">
                <div class="clearfix">
                {!! \App\Handlers\Admin\ManagePost\Post\CreateFormHandler::handler(request(), url()->full(), ['parent_post_id'=>$model_info->id]) !!}
                </div>
            </div>
        </div>
    </div>
...
<!-- Modal -->
<div class="modal fade" id="model_new_reported_posts" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true" style="background:rgba(0,0,0,.2);">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content" style="border:4px solid #1193d7;">
            <div class="modal-header bg-white text-primary py-2">
                <h5 class="modal-title">New reported post</h5>
                <button type="button" class="close py-1 my-0" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <div class="clearfix">
                {!! \App\Handlers\Admin\ManagePost\ReportedPost\CreateFormHandler::handler(request(), url()->full(), ['post_id'=>$model_info->id]) !!}
                </div>
            </div>
        </div>
    </div>

    ...
    <div class="container-header" style="overflow:visible;">
        <div class="d-flex align-items-center justify-content-between p-3">
            <div class="h5 m-0">
                ...
                <span>Subscriber info ( {{ $model_info->id }} )</span>
            </div>
            
            <div>
                <!-- <a href="{{ url('admin/subscriptions/subscribers/edit/' . $model_info->id) }}" class="btn btn-primary"><i class="fas fa-pencil-alt mr-1"></i> Edit</a> -->
                <a href="{{ url('admin/subscriptions/subscribers/create') }}" class="btn btn-dark"><i class="fas fa-plus-circle mr-1"></i>Add Subscriber</a>
                <a href="{{ url('admin/subscriptions/subscribers/delete/'. $model_info->id) }}?redirect={{ url('admin/subscriptions/subscribers/list') }}" class="btn btn-danger" data-confirmation='I you sure, you want to delete "{{ $model_info->id }}"?'><i class="fas fa-trash mr-1"></i> Delete</a>
            </div>
        </div>
    </div>
    <div class="container-detail bg-white">
        <div class="p-3">
            {!! \App\Handlers\Admin\Subscription\Subscriber\ShowFormHandler::handler(request(), new \App\Models\Subscriber(), $model_info->id) !!}
        </div>
    </div>
</div>

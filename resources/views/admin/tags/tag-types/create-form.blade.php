...
<form action="{{ url('admin/tags/tag-types/store'.(isset($redirect)? '?redirect='.$redirect: null)) }}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
...
window.form_children_input_value['insert_item_tags_tag_types_post_tags_list'] = {!! old('tags_tag_types_post_tags')? json_encode(old('tags_tag_types_post_tags')): '[{}]' !!};
...
window.form_children_input_value['insert_item_tags_tag_types_tags_list'] = {!! old('tags_tag_types_tags')? json_encode(old('tags_tag_types_tags')): '[{}]' !!};
...
window.form_children_input_template['insert_item_tags_tag_types_tags_list'] = function(i, random_id, data, error){ 
    ...
    var str = `
        <td>
            <input type="text" class="form-control ${ ((error && error.name)? 'is-invalid': '') }" name="tags_list[${random_id}][name]" value="${ ((data.name != null)? data.name:'') }" id="_input_name_${random_id}" placeholder="Name">
        </td>
        <td class="text-right" width="100">

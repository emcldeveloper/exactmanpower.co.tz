    ...
    <div class="container-header bg-white" style="overflow:visible;">
        <div class="row align-items-center justify-content-between p-3">
            <div class="col-12 col-md-6 mb-3 mb-md-0 h5 m-0">
                <a href="{{ url('admin/tags/tag-types/list') }}" class="btn btn-outline-primary px-2 mr-3" title="Back to tag types list"><i class="fas fa-arrow-left"></i></a>
                <span>Tag Type info ( {{ $model_info->name }} )</span>
            </div>
            
            <div class="col-12 col-md-6 d-none d-md-block text-right">
                <!-- <a href="{{ url('admin/tags/tag-types/edit/' . $model_info->tag_type_id) }}" class="btn btn-primary"><i class="fas fa-pencil-alt mr-1"></i> Edit</a> -->
                <a href="{{ url('admin/tags/tag-types/create') }}" class="btn btn-dark"><i class="fas fa-plus-circle mr-1"></i>Add Tag Type</a>
                <a href="{{ url('admin/tags/tag-types/delete/'. $model_info->tag_type_id) }}?redirect={{ url('admin/tags/tag-types/list') }}" class="btn btn-danger" data-confirmation='I you sure, you want to delete "{{ $model_info->name }}"?'><i class="fas fa-trash mr-1"></i> Delete</a>
            </div>
        </div>
    </div>
    <div class="container-detail">
        <div class="p-3">
            {!! \App\Handlers\Admin\Tag\TagType\ShowFormHandler::handler(request(), $model_info->tag_type_id) !!}
        </div>
    </div>
</div>

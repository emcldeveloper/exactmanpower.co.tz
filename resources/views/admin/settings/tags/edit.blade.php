...
<div class="main-container-middle">
    <div class="container-header" style="overflow:visible;">
        <div class="d-flex align-items-center justify-content-between p-3">
            <div class="h5 m-0">
                <a href="{{ url('admin/settings/tags/list') }}" class="btn btn-outline-primary mr-3" title="Back to tags list"><i class="fas fa-arrow-left"></i></a>
                <span>Edit tag ( {{ $model->name }} )</span>
            </div>
            <div>
                <a href="{{ url('admin/settings/tags/create') }}" class="btn btn-primary"><i class="fas fa-plus-circle mr-1"></i> Add new</a>
                <!-- <a href="{{ url('admin/settings/tags/list') }}" class="btn btn-primary"><i class="fas fa-list mr-1"></i> List</a> -->
                <a href="{{ url('admin/settings/tags/delete/'. $model->id) }}?redirect={{ url('admin/settings/tags/list') }}" class="btn btn-danger" data-confirmation='I you sure, you want to delete "{{ $model->name }}"?'><i class="fas fa-trash mr-1"></i> Delete</a>
            </div>
        </div>
    </div>
    <div class="container-detail bg-white">
        <div class="p-3">
            {!! \App\Handlers\Admin\Setting\Tag\EditFormHandler::handler(request(), new \App\Models\Tag(), $model->id) !!}
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content" style="border:4px solid #1193d7;">
            ...
            <div class="modal-body">
                <div class="clearfix">
                    {!! \App\Handlers\Admin\Setting\TagType\CreateFormHandler::handler(request(), null) !!}
                </div>
            </div>
        </div>
    </div>

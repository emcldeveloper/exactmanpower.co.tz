...
<form action="{{ url('admin/settings/locations/store'.(isset($redirect)? '?redirect='.$redirect: null)) }}" method="POST">
    {{ csrf_field() }}
    ...
    <div class="clearfix">
        ...
        @if(!isset($hidden) || (isset($hidden) && !in_array("parent_location_id", $hidden)))
        <div class="form-group">
            ...
            <select class="form-control {{ $errors->has('parent_location_id')? 'is-invalid': null }}" name="parent_location_id" id="_input_parent_location_id">
                ...
                <option value="{{ $row->parent_location_id }}" {{ ( (old('parent_location_id') == $row->parent_location_id)? 'selected':null ) }}>{{ $row->parent_location_id }}</option>
                @endforeach
            </select>

...
<div class="main-container-middle">
    <div class="container-header" style="overflow:visible;">
        <div class="d-flex align-items-center justify-content-between p-3">
            ...
            <div>
                <!-- <a href="{{ url('admin/settings/tag-types/edit/' . $model_info->id) }}" class="btn btn-primary"><i class="fas fa-pencil-alt mr-1"></i> Edit</a> -->
                <a href="{{ url('admin/settings/tag-types/create') }}" class="btn btn-dark"><i class="fas fa-plus-circle mr-1"></i>Add Tag Type</a>
                <a href="{{ url('admin/settings/tag-types/delete/'. $model_info->id) }}?redirect={{ url('admin/settings/tag-types/list') }}" class="btn btn-danger" data-confirmation='I you sure, you want to delete "{{ $model_info->name }}"?'><i class="fas fa-trash mr-1"></i> Delete</a>
            </div>
        </div>
    </div>
    <div class="container-detail bg-white">
        <div class="p-3">
            {!! \App\Handlers\Admin\Setting\TagType\ShowFormHandler::handler(request(), new \App\Models\TagType(), $model_info->id) !!}
        </div>
    </div>
</div>

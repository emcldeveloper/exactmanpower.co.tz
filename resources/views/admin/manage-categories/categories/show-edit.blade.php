...
<form class="edit-form p-3" action="{{ url($route.'/update/'.$model_info->id.(isset($redirect)? '?redirect='.$redirect: null)) }}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
    ...
    <div class="clearfix">
        ...
        <div class="clearfix mb-3 category-elements-item-container" data-children="category_elements_list">
            ...
            <div class="btn btn-light btn-block border add-item"><i class="fa fa-plus-circle" aria-hidden="true"></i> Add new field</div>
        </div>
...
window.form_children_input_value['insert_item_category_elements_list'] = {!! (($model_info->category_elements)? json_encode($model_info->category_elements): 'null') !!};

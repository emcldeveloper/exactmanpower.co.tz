...
<form class="edit-form" action="{{ url($route.'/store'.(isset($redirect)? '?redirect='.$redirect: null)) }}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
            ...
            <div class="col-12 col-lg-7">
                <div class="form-group">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="text" class="form-control {{ $errors->has('conditions')? 'is-invalid': null }} tags-input " name="conditions" value="{{ old('conditions') }}" placeholder="Conditions" id="_input_conditions">
                    </div>
            ...
            <div class="col-12 col-lg-4">
                ...
                <div class="form-group">
                    ...
                    <select data-input-control="" class="form-control {{ $errors->has('parent_category_id')? 'is-invalid': null }}" name="parent_category_id" id="_input_parent_category_id">
                        ...
                        <option value="{{ $row->category_id }}" {{ (old('parent_category_id') == $row->category_id)? 'selected':null }}>{{ $row->name }}</option>
                        @endforeach
                    </select>
...
window.form_children_input_value['insert_item_category_elements_list'] = {!! (old('category_elements')? json_encode(old('category_elements')): 'null') !!};

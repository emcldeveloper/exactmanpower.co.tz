...
<form action="{{ url('admin/manage-ads/packages/update/'.$model_info->id.(isset($redirect)? '?redirect='.$redirect: null)) }}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
    ...
    <div class="clearfix">
        ...
        <div class="form-group">
            <label class="font-weight-bold mb-1" for="featured_url">
                Featured Image
                @if($model_info->featured())
                <img src="{{ $model_info->featured() }}" height="20">
                @endif
            </label>
            <div class="custom-file">
                <input name="featured_url" type="file" class="custom-file-input" id="_input_featured_url">
                <label class="custom-file-label" for="_input_featured_url">
                
                @if($model_info->featured())
                {{ $model_info->featured() }}
                @else
                Choose featured image file
                @endif
                </label>
            </div>

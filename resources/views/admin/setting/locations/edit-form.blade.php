...
<form action="{{ url('admin/setting/locations/update/'.$model_info->location_id.(isset($redirect)? '?redirect='.$redirect: null)) }}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
    ...
    <div class="clearfix">
        ...
        <div class="form-group">
            ...
            <select class="form-control {{ $errors->has('parent_location_id')? 'is-invalid': null }}" name="parent_location_id" id="_input_parent_location_id">
                ...
                <option value="{{ $row->parent_location_id }}" {{ ( ($model_info->parent_location_id == $row->parent_location_id)? 'selected':null ) }}>{{ $row->name }}</option>
                @endforeach
            </select>

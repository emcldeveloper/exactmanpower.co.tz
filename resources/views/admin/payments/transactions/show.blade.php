...
<div class="main-container-middle">
    <div class="container-header" style="overflow:visible;">
        <div class="d-flex align-items-center justify-content-between p-3">
            <div class="h5 m-0">
                <a href="{{ url('admin/payments/transactions/list') }}" class="btn btn-outline-primary mr-3" title="Back to transactions list"><i class="fas fa-arrow-left"></i></a>
                <span>Transaction info ( {{ $model_info->id }} )</span>
            </div>
            
            <div>
                <!-- <a href="{{ url('admin/payments/transactions/edit/' . $model_info->id) }}" class="btn btn-primary"><i class="fas fa-pencil-alt mr-1"></i> Edit</a> -->
                <a href="{{ url('admin/payments/transactions/create') }}" class="btn btn-dark"><i class="fas fa-plus-circle mr-1"></i>Add Transaction</a>
                <a href="{{ url('admin/payments/transactions/delete/'. $model_info->id) }}?redirect={{ url('admin/payments/transactions/list') }}" class="btn btn-danger" data-confirmation='I you sure, you want to delete "{{ $model_info->id }}"?'><i class="fas fa-trash mr-1"></i> Delete</a>
            </div>
        </div>
    </div>
    <div class="container-detail bg-white">
        <div class="p-3">
            {!! \App\Handlers\Admin\Payment\Transaction\ShowFormHandler::handler(request(), new \App\Models\Transaction(), $model_info->id) !!}
        </div>
    </div>
</div>

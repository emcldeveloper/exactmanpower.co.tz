...
<div class="main-container-middle">
    <div class="container-header" style="overflow:visible;">
        <div class="d-flex align-items-center justify-content-between p-3">
            ...
            <div>
                <!-- <a href="{{ url('admin/logs/user-logs/edit/' . $model_info->id) }}" class="btn btn-primary"><i class="fas fa-pencil-alt mr-1"></i> Edit</a> -->
                <a href="{{ url('admin/logs/user-logs/create') }}" class="btn btn-dark"><i class="fas fa-plus-circle mr-1"></i>Add User Log</a>
                <a href="{{ url('admin/logs/user-logs/delete/'. $model_info->id) }}?redirect={{ url('admin/logs/user-logs/list') }}" class="btn btn-danger" data-confirmation='I you sure, you want to delete "{{ $model_info->id }}"?'><i class="fas fa-trash mr-1"></i> Delete</a>
            </div>
        </div>
    </div>
    <div class="container-detail bg-white">
        <div class="p-3">
            {!! \App\Handlers\Admin\Log\UserLog\ShowFormHandler::handler(request(), new \App\Models\UserLog(), $model_info->id) !!}
        </div>
    </div>
</div>

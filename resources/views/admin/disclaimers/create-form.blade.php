...
<form action="{{ url('admin/manage-ads/disclaimers/store'.(isset($redirect)? '?redirect='.$redirect: null)) }}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
    ...
    <div class="clearfix">
        ...
        <div class="form-group">
            ...
            <textarea type="text" class="form-control ckeditor {{ $errors->has('details')? 'is-invalid': null }}" name="details" placeholder="Details" id="_input_details">{{ old('details') }}</textarea>
        ...
        <div class="form-group">
            ...
            @php   
                $selected_categories = is_array(old('categories'))? old('categories'): [];
            @endphp
...
jQuery(function(){
    jQuery('.check-all').on('click', function(){
        jQuery('[name="categories[]"').attr('checked', true);
    });

    jQuery('.uncheck-all').on('click', function(){
        jQuery('[name="categories[]"').attr('checked', false);
    })

...
<div class="main-container-middle">
        ...
        <div class="main-container-middle">
            <div class="container-summary-sidebar border-bottom d-flex p-3">
                <div class="d-block">
                    ...
                    <span>{{ Str::title(request('post_type_id')) }}</span>
                    </div>
                    ...
                    <div class="container-detail" scroll-container>
                        <div class="clearfix custom-list-group">
                            ...
                            <div class="border-bottom custom-list-item p-2">
                                <a href="{{ url('admin/posts/'.request('post_type_id').'/show/'.$row->post_id.( request('sub_page')? '/'.request('sub_page'): '' )) }}">{{ $row->post_title }}</a>
    ...
    <div class="container-header bg-white" style="overflow:visible;">
        <div class="d-flex align-items-center justify-content-between p-3">
            <div class="h5 m-0">
                <span>{{ Str::title(request('post_type_id')) }} info ( {{ $model_info->post_title }} )</span>
            </div>
            
            <div>
                @if(false)
                <!-- <a href="{{ url('admin/posts/'.request('post_type_id').'/edit/'.$model_info->post_id) }}" class="btn btn-primary"><i class="fas fa-pencil-alt mr-1"></i> Edit</a> -->
                <a href="{{ url('admin/posts/'.request('post_type_id').'/create') }}" class="btn btn-dark"><i class="fas fa-plus-circle mr-1"></i> Add Post</a>
                <!-- <a href="{{ url('admin/posts/'.request('post_type_id').'/delete/'. $model_info->post_id) }}?redirect={{ url('admin/posts/'.request('post_type_id').'/list') }}" class="btn btn-danger" data-confirmation='I you sure, you want to delete "{{ $model_info->post_title }}"?'><i class="fas fa-trash mr-1"></i> Delete</a> -->
                <div class="d-inline-block dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" id="dropdown-add-another">
                        <div class="dropdown-item" data-toggle="modal" data-target="#model_new_post_comments" >Add post comment</div>
    ...
    <div class="container-navbar bg-white border-bottom" style="overflow:visible;">
        <nav class="nav">
            ...
            <a class="nav-link {{ Request::is('admin/posts/'.request('post_type_id').'/show/'.$model_info->post_id)? 'active': null }}" href="{{ url('admin/posts/'.request('post_type_id').'/show/' . $model_info->post_id) }}">Information</a>
            <a class="nav-link {{ Request::is('admin/posts/'.request('post_type_id').'/show/*/post-comments')? 'active': null }}" href="{{ url('admin/posts/'.request('post_type_id').'/show/' . $model_info->post_id . '/post-comments') }}">Post Comments</a>
            <a class="nav-link {{ Request::is('admin/posts/'.request('post_type_id').'/show/*/posts')? 'active': null }}" href="{{ url('admin/posts/'.request('post_type_id').'/show/' . $model_info->post_id . '/posts') }}">Posts</a>
        </nav>
    </div>
    <div class="container-detail">
        <div class="clearfix">
            {!! \App\Handlers\Admin\Post\Post\ShowFormHandler::handler(request(), request('post_type_id'), $model_info->post_id) !!}
        </div>
    </div>
</div>
<!-- Modal -->
<div class="modal fade" id="model_new_post_comments" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true" style="background:rgba(0,0,0,.2);">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content" style="border:4px solid #1193d7;">
            <div class="modal-header bg-white text-primary py-2">
                <h5 class="modal-title">New post comment</h5>
            ...
            <div class="modal-body">
                <div class="clearfix">
                {!! \App\Handlers\Admin\Post\PostComment\CreateFormHandler::handler(request(), false, url()->full(), ['post_id'=>$model_info->post_id]) !!}
                </div>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="model_new_posts" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true" style="background:rgba(0,0,0,.2);">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content" style="border:4px solid #1193d7;">
            ...
            <div class="modal-body">
                <div class="clearfix">
                {!! \App\Handlers\Admin\Post\Post\CreateFormHandler::handler(request(), false, url()->full(), ['parent_post_id'=>$model_info->post_id]) !!}
                </div>
            </div>
        </div>
    </div>

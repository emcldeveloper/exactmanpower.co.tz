...
<div class="clearfix">
    <div class="px-3 pb-3">
        ...
        <div class="row align-items-center justify-content-between m-0">
            <div class="col-12 col-md-6 p-0 mb-3 mb-md-0">
                {!! pagination_header_limit($post_comments_list) !!}
            </div>
            <div class="col-12 col-md-6 p-0 mb-3 mb-md-0">
                {!! pagination_header_search($post_comments_list) !!}
            </div>
        </div>
    </div>
    ...
    <div class="px-3">
        @if(true)
        @foreach ($post_comments_list as $index => $row)
        <div class="card zoom hover-container box-shadow rounded-0 py-2 px-4 mb-2">
            <div class="row align-items-center">
                <div class="col pr-0" style="max-width:50px;">{{ ($post_comments_list->firstItem() + $index) }}</div>
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col">
                            <div class="row">
                                <div class="col">
                                    @if($row->post)
                                    <a class="d-block" href="{{ url('admin/posts/posts/show/'. $row->post->post_id) }}">
                                        {{ $row->post->post_title }}
                                    </a>
                                    @endif
                                </div>
        ...
        <div class="border bg-white">
            <table class="table table-hover table-sm mb-0">
                ...
                @foreach ($post_comments_list as $index => $row)
                    <tr>
                        <td scope="row">{{ ($post_comments_list->firstItem() + $index) }}</td>
                        <td>
                            @if($row->post)
                            <a class="d-block" href="{{ url('admin/posts/posts/show/'. $row->post->post_id) }}">
                                <?= $row->post->post_title;?>
                        ...
                        <td class="text-right">
                            <div class="btn-group btn-group-sm">
                                <a href="{{ url('admin/posts/post-comments/show/'. $row->post_comment_id) }}" class="btn px-1 py-0"> <i class="fas fa-file"></i> </a>
                                <a href="{{ url('admin/posts/post-comments/edit/'. $row->post_comment_id) }}" class="btn px-1 py-0"> <i class="fas fa-pencil-alt"></i> </a>
                                <a href="{{ url('admin/posts/post-comments/delete/'. $row->post_comment_id) }}?redirect={{ url()->full() }}" class="btn px-1 py-0 text-danger" data-confirmation='I you sure, you want to delete "{{ $row->post_id }}"?'> <i class="fas fa-trash"></i> </a>
                            </div>
                        </td>
                    </tr>
                @endforeach
                </tbody>
                @if($post_comments_list->count() > 0)
        ...
        {!! pagination_footer($post_comments_list) !!}
    </div>
</div>
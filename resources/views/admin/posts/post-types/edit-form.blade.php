...
<form action="{{ url('admin/posts/post-types/update/'.$model_info->post_type_id.(isset($redirect)? '?redirect='.$redirect: null)) }}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
...
window.form_children_input_value['insert_item_posts_post_types_metas_list'] = {!! old('posts_post_types_metas')? json_encode(old('posts_post_types_metas')): json_encode($model_info->metas) !!};
...
window.form_children_input_template['insert_item_posts_post_types_metas_list'] = function(i, random_id, data, error){ 
    ...
    var str = `
        <td>
            <input type="text" class="form-control ${ ((error && error.name)? 'is-invalid': '') }" name="metas_list[${random_id}][name]" value="${ ((data.name)? data.name:'') }" id="_input_name_${random_id}" placeholder="Name">
        </td>
        <td>
            <input type="text" class="form-control ${ ((error && error.input_type)? 'is-invalid': '') }" name="metas_list[${random_id}][input_type]" value="${ ((data.input_type)? data.input_type:'') }" id="_input_input_type_${random_id}" placeholder="Input Type">
        </td>
        <td>
            <input type="text" class="form-control ${ ((error && error.multiple)? 'is-invalid': '') }" name="metas_list[${random_id}][multiple]" value="${ ((data.multiple)? data.multiple:'') }" id="_input_multiple_${random_id}" placeholder="Multiple">
        </td>
        <td>
            <input type="text" class="form-control ${ ((error && error.options)? 'is-invalid': '') }" name="metas_list[${random_id}][options]" value="${ ((data.options)? data.options:'') }" id="_input_options_${random_id}" placeholder="Options">
        </td>
        <td>
            <input type="text" class="form-control ${ ((error && error.linked_type_id)? 'is-invalid': '') }" name="metas_list[${random_id}][linked_type_id]" value="${ ((data.linked_type_id)? data.linked_type_id:'') }" id="_input_linked_type_id_${random_id}" placeholder="Linked Type">
        </td>

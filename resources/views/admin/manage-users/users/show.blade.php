...
<div class="main-container-middle">
    <div class="container-sidebar bg-white border-left border-right">
        <div class="main-container-middle">
            <div class="container-summary-sidebar border-bottom d-flex p-3">
                <div class="d-block">
                    <div class="h5 mb-1">
                    <!-- <a href="{{ url('admin/manage-users/users/list') }}" class="btn btn-outline-primary mr-3" title="Back to users list"><i class="fas fa-arrow-left"></i></a> -->
                    <span>Users</span>
                    </div>
                ...
                <div class="main-container-middle">
                    <div class="container-detail" scroll-container>
                        <div class="clearfix custom-list-group">
                            ...
                            <a href="{{ url('admin/manage-users/users/show/'.$row->user_id.( request('sub_page')? '/'.request('sub_page'): '' )) }}" class="d-block border-bottom custom-list-item p-2">
    ...
    <div class="container-header bg-white" style="overflow:visible;">
        <div class="d-flex align-items-center justify-content-between p-3">
            <div class="h5 m-0">
                <span>User info ( {{ $model_info->first_name }} )</span>
            </div>
            
            <div>
                @if(false)
                <!-- <a href="{{ url('admin/manage-users/users/edit/'.$model_info->user_id) }}" class="btn btn-primary"><i class="fas fa-pencil-alt mr-1"></i> Edit</a> -->
                <a href="{{ url('admin/manage-users/users/create') }}" class="btn btn-dark"><i class="fas fa-plus-circle mr-1"></i> Add User</a>
                <!-- <a href="{{ url('admin/manage-users/users/delete/'. $model_info->user_id) }}?redirect={{ url('admin/manage-users/users/list') }}" class="btn btn-danger" data-confirmation='I you sure, you want to delete "{{ $model_info->first_name }}"?'><i class="fas fa-trash mr-1"></i> Delete</a> -->
    ...
    <div class="container-navbar bg-white border-bottom" style="overflow:visible;">
        <nav class="nav">
            ...
            <a class="nav-link {{ Request::is('admin/manage-users/users/show/'.$model_info->user_id)? 'active': null }}" href="{{ url('admin/manage-users/users/show/' . $model_info->user_id) }}">Information</a>
            <a class="nav-link {{ Request::is('admin/manage-users/users/show/*/posts')? 'active': null }}" href="{{ url('admin/manage-users/users/show/' . $model_info->user_id . '/posts') }}">Posts</a>
        </nav>
    </div>
    <div class="container-detail">
        <div class="clearfix">
            {!! \App\Handlers\Admin\ManageUser\User\ShowFormHandler::handler(request(), $model_info->user_id) !!}
        </div>
    </div>
</div>
<!-- Modal -->
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content" style="border:4px solid #1193d7;">
            ...
            <div class="modal-body">
                <div class="clearfix">
                {!! \App\Handlers\Admin\Post\Post\CreateFormHandler::handler(request(), false, url()->full(), ['post_author'=>$model_info->user_id]) !!}
                </div>
            </div>
        </div>
    </div>

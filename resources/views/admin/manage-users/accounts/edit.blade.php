    ...
    <div class="container-header" style="overflow:visible;">
        <div class="d-flex align-items-center justify-content-between p-3">
            <div class="h5 m-0">
                ...
                <span>Edit account ( {{ $model->name }} )</span>
            </div>
            <div>
                <a href="{{ url('admin/manage-users/accounts/create') }}" class="btn btn-primary"><i class="fas fa-plus-circle mr-1"></i> Add new</a>
                <!-- <a href="{{ url('admin/manage-users/accounts/list') }}" class="btn btn-primary"><i class="fas fa-list mr-1"></i> List</a> -->
                <a href="{{ url('admin/manage-users/accounts/delete/'. $model->id) }}?redirect={{ url('admin/manage-users/accounts/list') }}" class="btn btn-danger" data-confirmation='I you sure, you want to delete "{{ $model->name }}"?'><i class="fas fa-trash mr-1"></i> Delete</a>
            </div>
        </div>
    </div>
    <div class="container-detail bg-white">
        <div class="p-3">
            {!! \App\Handlers\Admin\ManageUser\Account\EditFormHandler::handler(request(), new \App\Models\Account(), $model->id) !!}
        </div>
    </div>
</div>

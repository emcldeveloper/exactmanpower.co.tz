    ...
    <div class="container-header bg-white" style="overflow:visible;">
        <div class="row align-items-center justify-content-between p-3">
            <div class="col-12 col-md-6 mb-3 mb-md-0 h5 m-0">
                <a href="{{ url('admin/manage-users/logs/list') }}" class="btn btn-outline-primary px-2 mr-3" title="Back to logs list"><i class="fas fa-arrow-left"></i></a>
                <span>Log info ( {{ $model_info->name }} )</span>
            </div>
            
            <div class="col-12 col-md-6 d-none d-md-block text-right">
                <!-- <a href="{{ url('admin/manage-users/logs/edit/' . $model_info->log_id) }}" class="btn btn-primary"><i class="fas fa-pencil-alt mr-1"></i> Edit</a> -->
                <a href="{{ url('admin/manage-users/logs/create') }}" class="btn btn-dark"><i class="fas fa-plus-circle mr-1"></i>Add Log</a>
                <a href="{{ url('admin/manage-users/logs/delete/'. $model_info->log_id) }}?redirect={{ url('admin/manage-users/logs/list') }}" class="btn btn-danger" data-confirmation='I you sure, you want to delete "{{ $model_info->name }}"?'><i class="fas fa-trash mr-1"></i> Delete</a>
            </div>
        </div>
    </div>
    <div class="container-detail">
        <div class="p-3">
            {!! \App\Handlers\Admin\ManageUser\Log\ShowFormHandler::handler(request(), $model_info->log_id) !!}
        </div>
    </div>
</div>

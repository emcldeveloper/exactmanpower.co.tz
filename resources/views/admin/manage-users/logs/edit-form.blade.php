...
<form action="{{ url('admin/manage-users/logs/update/'.$model_info->log_id.(isset($redirect)? '?redirect='.$redirect: null)) }}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
...
window.form_children_input_value['insert_item_manage_users_logs_user_logs_list'] = {!! old('manage_users_logs_user_logs')? json_encode(old('manage_users_logs_user_logs')): json_encode($model_info->user_logs) !!};

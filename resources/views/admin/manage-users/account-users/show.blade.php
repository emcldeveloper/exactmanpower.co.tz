    ...
    <div class="container-header" style="overflow:visible;">
        <div class="d-flex align-items-center justify-content-between p-3">
            <div class="h5 m-0">
                ...
                <span>Account User info ( {{ $model_info->id }} )</span>
            </div>
            
            <div>
                <!-- <a href="{{ url('admin/manage-users/account-users/edit/' . $model_info->id) }}" class="btn btn-primary"><i class="fas fa-pencil-alt mr-1"></i> Edit</a> -->
                <a href="{{ url('admin/manage-users/account-users/create') }}" class="btn btn-dark"><i class="fas fa-plus-circle mr-1"></i>Add Account User</a>
                <a href="{{ url('admin/manage-users/account-users/delete/'. $model_info->id) }}?redirect={{ url('admin/manage-users/account-users/list') }}" class="btn btn-danger" data-confirmation='I you sure, you want to delete "{{ $model_info->id }}"?'><i class="fas fa-trash mr-1"></i> Delete</a>
            </div>
        </div>
    </div>
    <div class="container-detail bg-white">
        <div class="p-3">
            {!! \App\Handlers\Admin\ManageUser\AccountUser\ShowFormHandler::handler(request(), new \App\Models\AccountUser(), $model_info->id) !!}
        </div>
    </div>
</div>

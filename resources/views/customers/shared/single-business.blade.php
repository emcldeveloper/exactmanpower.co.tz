...
<div class="card p-4">
    ...
    @if($post->timetables->count())
    @foreach($post->timetables as $index => $row)
        <div class="row">
            <div class="col-3">{{ $row->day_name() }}</div>
            <div class="col">
            {!! $row->availability() !!}
            </div>
        </div>
    @endforeach
    @else
        <span>Working hours not spacified</span>
    @endif
    </div>
</div>
...
<div class="card mt-4 p-4">
    ...
    <hr>
    <div class="secondary-font">
        <a href="{{ url('download/'.str_replace('.', '-dot-', $post->document_url)) }}" target="_blank">
            Download 
            @if($post->document_type == Post::DOC_BROCHURE)
                brochure
            @elseif($post->document_type == Post::DOC_MENU)
                menu
            @elseif($post->document_type == Post::DOC_FLYER)
                flyer
            @elseif($post->document_type == Post::DOC_COMPANY_PROFILE)
                company profile
            @elseif($post->document_type == Post::DOC_ADVERT)
                advert
            @else
                document
            @endif

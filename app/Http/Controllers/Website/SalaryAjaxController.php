...
class SalaryAjaxController extends Controller
{
    public function calculate(Request $request)
    {
        ...
        // Usage Counter 
        $key = $salaryType === 'net' ? $net_pay : $basic_pay;

        $lastInput = session('last_input_value');

        if ($key && $key != $lastInput) {

            $row = SalaryUsageSummary::firstOrCreate(
                ['type' => $salaryType],
                ['view_count' => 0]
            );
        ...
        $usage_count = SalaryUsageSummary::sum('view_count');

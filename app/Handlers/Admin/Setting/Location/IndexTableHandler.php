...
class IndexTableHandler
{
    ...
    public static function handler(Request $request, $api = false)
    {
        ...
        if($search){
            ...
            $model->where(function($query) use($search){
                $query->where('id', 'LIKE', "%".$search."%") // match Id column
                    ...
                    ->orWhere('parent_location_id', 'LIKE', "%".$search."%") // match Parent Location column
                    ->orWhere('type', 'LIKE', "%".$search."%") // match Type column
                    ->orWhere('latitude', 'LIKE', "%".$search."%") // match Latitude column
                    ->orWhere('longitude', 'LIKE', "%".$search."%"); // match Longitude column
            });
        }
        ...
        $data['locations_list'] = $paginate_list;
        ...
        return view('admin.setting.locations.index-table', $data);
    }
}